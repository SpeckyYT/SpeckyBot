...
module.exports.call = async (bot, oldMember, newMember) => {
    ...
    let removeRoles = ['607522930800459789','269841158694371329','605392884526612524','599988443476525087','309678822435848204','268409019641626626','268409297338105867','265519525041143809']
    let addRoles = ['638091940545560639']
                    ...
                    [game.name, game.state].forEach(item => {
                        if(item){
                            if(item.toLowerCase().match(regex)){
                                try{member.addRole(addRoles)}catch(e){}
                                try{member.removeRole(removeRoles)}catch(e){}
                            }
                        }
                    })
                }
